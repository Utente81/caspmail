...
export default async function adminRoutes(app) {
  ...
  // Throws 403 if the actor (non-global-admin) does not own targetTenantId.
  // Set globalOnly=true to block any non-casper_admin completely.
  async function assertTenantAccess(req, targetTenantId, { globalOnly = false } = {}) {
    const isGlobal = req.user?.realm_access?.roles?.includes('casper_admin');
    if (isGlobal) return; // casper_admin bypasses all checks
    if (globalOnly) throw Object.assign(new Error('Only casper_admin can perform this action'), { statusCode: 403 });
    const actorTenant = await getTenantId(req);
    if (!actorTenant) throw Object.assign(new Error('No tenant association'), { statusCode: 403 });
    if (actorTenant !== targetTenantId) throw Object.assign(new Error('Forbidden: cross-tenant access denied'), { statusCode: 403 });
  }

  // Convenience: extract tenant and assert in one call; returns the actorTenantId.
  async function requireTenant(req) {
    const isGlobal = req.user?.realm_access?.roles?.includes('casper_admin');
    if (isGlobal) return null; // returns null signals "global admin, no restriction"
    const tenantId = await getTenantId(req);
    if (!tenantId) throw Object.assign(new Error('No tenant association'), { statusCode: 403 });
    return tenantId;
  }
  ...
  app.get('/users', adminGuard, async (req, reply) => {
    ...
    let tenantId;
    if (isGlobalAdmin) {
      tenantId = req.query.tenant_id || null; // optional filter for global admins
    } else {
      tenantId = await getTenantId(req);
      if (!tenantId) return reply.status(403).send({ error: 'No tenant association' });
    }

    let query = 'SELECT id, tenant_id, email, name, role, quota_mb, status, legal_hold, created_at FROM users';
    const params = [];
    const where = [];
  ...
  app.post('/users', adminGuard, async (req, reply) => {
    ...
    try { await assertTenantAccess(req, tenant_id); } catch (e) { return reply.status(e.statusCode || 403).send({ error: e.message }); }
    if (!tenant_id || !email) {
  ...
  app.patch('/users/:id/legal-hold', adminGuard, async (req, reply) => {
    ...
    try {
      // Load user first to validate tenant ownership
      const { rows: existing } = await pool.query('SELECT tenant_id FROM users WHERE id = $1', [id]);
      if (existing.length === 0) return reply.status(404).send({ error: 'User not found' });
      await assertTenantAccess(req, existing[0].tenant_id);
      const { rows } = await pool.query(
        'UPDATE users SET legal_hold = $1 WHERE id = $2 RETURNING id, email, legal_hold, tenant_id',
        [legal_hold, id]
      );
  ...
  app.post('/domains/:id/verify', adminGuard, async (req, reply) => {
    ...
    try { await assertTenantAccess(req, domain.tenant_id); } catch (e) { return reply.status(e.statusCode || 403).send({ error: e.message }); }
