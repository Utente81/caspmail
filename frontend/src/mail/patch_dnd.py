...
dash_target2 = '''              <div key={f.id} className={mail-nav-item} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}} onClick={() => setActive(f.id)}>'''

dash_replacement2 = '''              <div key={f.id} className={mail-nav-item} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}} onClick={() => setActive(f.id)}
                onDragOver={(e) => { e.preventDefault(); e.currentTarget.classList.add('drag-over'); }}
                onDragLeave={(e) => e.currentTarget.classList.remove('drag-over')}
                onDrop={(e) => {
                  e.preventDefault();
